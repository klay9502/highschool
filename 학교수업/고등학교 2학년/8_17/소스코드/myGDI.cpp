...
void DrawBigDot(int x, int y, int color)
{
	SetPixel(hdc, x - 1, y - 1, colors[color]);
	SetPixel(hdc, x, y - 1, colors[color]);
	SetPixel(hdc, x + 1, y - 1, colors[color]);
	SetPixel(hdc, x - 1, y, colors[color]);
	SetPixel(hdc, x, y, colors[color]);
	SetPixel(hdc, x + 1, y, colors[color]);
	SetPixel(hdc, x - 1, y + 1, colors[color]);
	SetPixel(hdc, x, y + 1, colors[color]);
...
void DrawBackground(void)
{
	...
	Line(rtScreen.right / 2, 0, rtScreen.right / 2, rtScreen.bottom);
	Line(0, rtScreen.bottom / 2, rtScreen.right, rtScreen.bottom / 2);
}

void DrawBackgroundGird(void)
{
	int x = 0, y = 0;
	WhitePen();

	for (int i = 0; i < rtScreen.right;)
	{
		if (x >= 5)
		{
			WhitePen();
...
void DrawBackgroundExCross(void)
{
	int num = 0, textnum = 50;
	char string[100];
	...
	for (int i = rtScreen.right / 2; i > 0; i -= 10)
	{
		if (num >= 5)
		{
			WhitePen();
			TextColor(yellow);
			Line(i, (rtScreen.bottom / 2) - 10, i, (rtScreen.bottom / 2) + 10);
			num = 0;
			itoa(textnum, string, 10);
			TextOut(hdc, i - 10, (rtScreen.bottom / 2) + 15, string, strlen(string));
			textnum += 50;
		}
		else
		{
			GrayPen();
			Line(i, (rtScreen.bottom / 2) - 5, i, (rtScreen.bottom / 2) + 5);
		}
		num++;
	}
	...
	for (int i = rtScreen.right / 2; i < rtScreen.right; i += 10)
	{
		if (num >= 5)
		{
			WhitePen();
	...
	for (int i = rtScreen.bottom / 2; i > 0; i -= 10)
	{
		if (num >= 5)
		{
			WhitePen();
			Line((rtScreen.right / 2) - 10, i, (rtScreen.right / 2) + 10, i);
			num = 0;
			itoa(textnum, string, 10);
			TextOut(hdc, (rtScreen.right / 2) + 15, i - 12, string, strlen(string));
			textnum += 50;
		}
		else
		{
			GrayPen();
			Line((rtScreen.right / 2) - 5, i, (rtScreen.right / 2) + 5, i);
		}
		num++;
	}
	...
	for (int i = rtScreen.bottom / 2; i < rtScreen.bottom; i += 10)
	{
		if (num >= 5)
		{
			WhitePen();
			...
			num = 0;
			itoa(textnum, string, 10);
			TextOut(hdc, (rtScreen.right / 2) + 15, i - 5, string, strlen(string));
			textnum += 50;
		}
		else
		{
